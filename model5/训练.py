...
val_log_path = args.val_log_path
...
for e in range(epoch, epochs):
    ...
    for b in tqdm(range(train_tensor.shape[0]), desc='训练批次中'):
        ...
        loss = mse_loss(p1_img, img)
        train_loss += loss.item()
        loss.backward()
        optimizer.step()
    print('训练轮数{},每幅图片训练损失:{}'.format(e, train_loss / (train_tensor.shape[0] * train_tensor.shape[1])))
    with open(train_log_path, 'a+', encoding='utf') as log:
        log.write('epoch:{},test_loss:{}\n'.
                  format(epoch, train_loss / (train_tensor.shape[0] * train_tensor.shape[1])))
    ...
    for b in tqdm(range(val_tensor.shape[0]), desc='验证批次中'):
        ...
        loss = mse_loss(p1_img, img)
        val_loss += loss.item()
    print('验证轮数{},每幅图片验证损失:{}'.format(e, val_loss / (val_tensor.shape[0] * val_tensor.shape[1])))
    with open(val_log_path, 'a+', encoding='utf') as log:
        log.write('epoch:{},test_loss:{}\n'.
                  format(epoch, val_loss / (val_tensor.shape[0] * val_tensor.shape[1])))
